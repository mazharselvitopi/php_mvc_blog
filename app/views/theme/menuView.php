...
<header class="bg-warning p-2 shadow rounded-3 border border-2 border-dark">
...
<ul class="dropdown-menu position-static d-grid gap-1 mt-2 p-2 rounded-3 mx-0 shadow border border-2 border-dark">
    <?php if ($params['now_controller'] == 'mainController' && $params['now_action'] == 'indexActionGetRequest' && !isset ($params['now_category'])) {?>
        <li><a class="dropdown-item rounded-2 active bg-warning text-black border border-1 border-dark " href="<?=$config['root_url']?>main/index">Tum Yazilar</a></li>
    <?php } else {?>
    ...
    <?php foreach ($params['categories'] as $category) {
        if(isset ($params['now_category'])) {
            if($params['now_category'] == $category->getId()) { ?>
                <li><a class="dropdown-item rounded-2 active bg-warning text-black border border-1 border-dark " href="<?=$config['root_url']?>main/index/category/<?=$category->getId()?>/page/1"><?=$category->getTitle()?></a></li>
            <?php
            } else { ?>
